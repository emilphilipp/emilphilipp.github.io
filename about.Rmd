...
brb_stations %>% dplyr::select(name,elevation,distance_750) %>% st_drop_geometry() %>% reactable::reactable(
  ...
  columns = list(
    ...
    distance_750 = colDef(name = "Distance to closest point 750 m higher (m)")
  ))
```
...
```
As for the second extension, this table shows which peaks can be reached along the trail network:
```{r echo=F }
#####
